...
class Booking extends CI_Controller
{
    ...
    public function tambahBooking()
    {
        ...
        $tempuser = $this->db->query("select*from temp where id_user ='$id_user'")->num_rows();
        //cek jika masih ada booking buku yang belum diambil
        $databooking = $this->db->query("select*from booking where id_user='$id_user'")->num_rows();
        if ($databooking > 0) {
    ...
    public function bookingSelesai()
    {
        ...
        $this->ModelBooking->simpanDetail('id_user');
    ...
    {
        $id_user = $this->session->userdata('id_user');
        $data['user'] = $this->ModelUser->get_where('user', ['nama' => $this->session->userdata('nama')])->row_array();
